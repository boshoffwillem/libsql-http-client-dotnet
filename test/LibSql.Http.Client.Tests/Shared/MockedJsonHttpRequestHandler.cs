...
public record MockedJsonHttpRequest(
    HttpMethod Method,
    string? Uri,
    string? Token,
    string? Body = null
...
public class MockedJsonHttpResponse(HttpStatusCode statusCode, object? body)
{
    ...
    public Task<HttpContent> GetContentAsync() =>
        Task.FromResult<HttpContent>(JsonContent.Create(body));

    public static implicit operator MockedJsonHttpResponse(HttpStatusCode statusCode) =>
        new(statusCode, null);

    public static implicit operator MockedJsonHttpResponse(JsonElement body) =>
...
public class MockedJsonHttpHandler(MockedJsonHttpResponse expectedResponse) : HttpMessageHandler
{
    ...
    )
    {
        ...
        return new HttpResponseMessage(expectedResponse.StatusCode) { Content = content };
    }

    public static implicit operator HttpClient(MockedJsonHttpHandler handler) =>
        new(handler) { BaseAddress = DefaultBaseAddress };
}
