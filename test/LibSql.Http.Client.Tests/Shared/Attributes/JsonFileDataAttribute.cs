...
public class JsonFileDataAttribute(string filePath, bool mapByProps = false) : DataAttribute
{
    public override IEnumerable<object?[]> GetData(MethodInfo testMethod)
    {
        ...
        return mapByProps
            ? GetMappingByProps(fileData, parameters)
            : GetAsArray(fileData, parameters);
    }
    ...
    private IEnumerable<object?[]> GetAsArray(string fileData, ParameterInfo[] parameters)
    {
        ...
        return deserialized?.Select(c =>
                c.Select(
                        (el, idx) =>
                            idx >= parameters.Length
                                ? el
                                : el.Deserialize(parameters[idx].ParameterType)
                    )
                    .ToArray()
            )
            ?? [];
    ...
    private IEnumerable<object?[]> GetMappingByProps(string fileData, ParameterInfo[] parameters)
    {
        ...
        [
            parameters
                .Select(p =>
                    p.Name is not null
                    && deserialized.RootElement.TryGetProperty(p.Name, out var val)
