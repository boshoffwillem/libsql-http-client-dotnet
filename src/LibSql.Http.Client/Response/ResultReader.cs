...
) : IResultReader
{
    ...
    public IEnumerable<T> Read<T>(JsonTypeInfo<T> typeInfo)
    {
        return ReadAt(_position, typeInfo);
    }

    public IEnumerable<T> ReadAt<T>(int index, JsonTypeInfo<T> typeInfo)
    {
        int uIndex = index < 0 ? Count + index : index;
    ...
    public object? GetScalarValue()
    {
        ...
        long[] row = rowsMarkers[0][0];

        Utf8JsonReader reader = new(buffer.AsSpan(row));
        ...
        while (reader.Read())
        {
            ...
            return reader.TokenType switch
            {
                ...
                JsonTokenType.Number => reader.TryGetInt64(out long i) ? i : reader.GetDouble(),
    ...
    {
        byte[] memoryOwner = ArrayPool<byte>.Shared.Rent((int)stream.Length);

        PooledByteBufferWriter ms = new();
        await using Utf8JsonWriter writer = new(ms);
        ...
        try
        {
            ...
            int read = await stream.ReadAsync(memoryOwner, cancellationToken);

            List<List<long[]>> rowsMarks = new();
            List<ExecutionError> errors = new();
            List<ExecutionStats> stats = new();

            string? baton = Initialize(
    ...
    private static void HandleErrorResult(ref Utf8JsonReader reader, List<ExecutionError> errors)
    {
        ...
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            bool isPropName = reader.TokenType == JsonTokenType.PropertyName;
        ...
        while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
        {
            ...
            if (reader.ValueTextEquals("rows"u8) && reader.Read())
            {
                while (reader.Read() && reader.TokenType != JsonTokenType.EndArray)
                {
                    ...
                    long init = writer.BytesCommitted + writer.BytesPending - 1;
                    ...
                    long length = writer.BytesCommitted + writer.BytesPending - init;
